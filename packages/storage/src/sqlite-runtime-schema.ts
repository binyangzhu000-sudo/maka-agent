...
export const SQLITE_RUNTIME_REQUIRED_TABLES = [
  ...
  ['tool_operations', 1],
  ['runtime_partial_snapshots', 2],
  ['runtime_partial_segments', 10],
  ['runtime_capabilities', 5],
  ['runtime_storage_root_binding', 9],
  ['runtime_continuation_claims', 6],
  ['runtime_workspace_epochs', 7],
  ['runtime_workspace_versions', 7],
  ['runtime_workspace_heads', 7],
  ['headless_task_run_events', 8],
...
const MIGRATIONS: ReadonlyMap<number, string> = new Map([
  ...
  [
    ...
    SELECT
      events.session_id,
      (
        SELECT COALESCE(MAX(existing.ordinal), 0)
        FROM runtime_session_event_ordinals existing
        WHERE existing.session_id = events.session_id
      ) + ROW_NUMBER() OVER (
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM runtime_session_event_ordinals existing
      WHERE existing.event_id = events.event_id
    );
